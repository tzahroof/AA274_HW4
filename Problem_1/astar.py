...
class AStar(object):
            ...
            for i in range(-1, 2):
                for j in range(-1, 2):
                    if not(i is 0 and j is 0):
                        possState = (x[0] + i*self.resolution, x[1] + j*self.resolution)

                        if(self.is_free(possState)):
                            neighbors.append(possState)
        ...
        def solve(self):
            ...
            #g-score = cost (path length)
            #f-score = cost + heuristic (how to select node to build path)

            #pop the node with the lowest f-score
                current_node = self.find_best_f_score()
                self.open_set.remove(current_node)
                self.closed_set.append(current_node)

                if(current_node == self.x_goal):
                ...
                for neighbor in self.get_neighbors(current_node):
                    ...
                    new_cost = self.g_score[current_node] + self.distance(current_node, neighbor)

                    if (neighbor not in self.g_score or new_cost < self.g_score[neighbor]):
                        if(neighbor not in self.open_set):
                            self.open_set.append(neighbor)

                        self.g_score[neighbor] = new_cost
                        self.f_score[neighbor] = new_cost + self.distance(neighbor, self.x_goal)
